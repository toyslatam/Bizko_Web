  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<ProfessionalFormSheet companyId={companyId} />}
      />

      {professionals.length > 0 || hasFilters ? (
        <div className="space-y-4">
          <ListToolbar searchPlaceholder="Buscar profesional..." />
          {professionals.length > 0 ? (
            ...
            <EmptyState
              ...
              description="No encontramos profesionales con esos filtros."
            />
          )}
        </div>
      ) : (
        <EmptyState
          icon={Scissors}
          title="Todavía no tienes profesionales"
          description="Agrega un profesional para asignarle servicios y comisiones."
          action={<ProfessionalFormSheet companyId={companyId} />}
        />
      )}
    </div>
  );
