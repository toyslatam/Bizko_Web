-- =============================================================================
-- Corrige un segundo bug real: create_public_order() validaba el stock de un
-- producto SIN variantes antes de aceptar el pedido, pero nunca lo
-- descontaba de products.current_stock al crear las líneas del pedido — solo
-- la rama de variantes (product_variants.stock) se actualizaba. Un pedido
-- público con productos normales (la mayoría de los negocios) dejaba el
-- inventario intacto.
--
-- No se usa apply_inventory_movement() aquí (a propósito, igual que ya hacía
-- la rama de variantes con un UPDATE directo): esa función valida
-- is_company_member(auth.uid()), pero create_public_order la invoca un
-- cliente anónimo del catálogo — fallaría la autorización. En su lugar se
-- inserta el movimiento de inventario directamente, igual que hace
-- apply_inventory_movement() internamente, para mantener la trazabilidad en
-- Reportes → Inventario.
-- =============================================================================
